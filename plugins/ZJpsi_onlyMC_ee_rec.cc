...
class ZJpsi_onlyMC_ee_rec : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit ZJpsi_onlyMC_ee_rec(const edm::ParameterSet&);
      ~ZJpsi_onlyMC_ee_rec();
   ...
   private:
      ...
      void analyzeDecay(const reco::Candidate*, TLorentzVector& ,TLorentzVector&, TLorentzVector&, TLorentzVector&,
...
//
ZJpsi_onlyMC_ee_rec::ZJpsi_onlyMC_ee_rec(const edm::ParameterSet& iConfig)
{
...
ZJpsi_onlyMC_ee_rec::~ZJpsi_onlyMC_ee_rec()
{
...
void
ZJpsi_onlyMC_ee_rec::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   //MC
...
std::string ZJpsi_onlyMC_ee_rec::printName(int pdgid){
    /*
    std::string retstr;
    if(pdgid == 11){
        retstr = "e-";
...
void ZJpsi_onlyMC_ee_rec::printMCtree(const reco::Candidate* mother, int indent=0){
    if (mother == NULL){
...
void ZJpsi_onlyMC_ee_rec::printMCtreeUP(const reco::Candidate* daughter, int indent = 0){
    if (daughter == NULL){
        std::cout << "end tree" << std::endl;
        return;
    }
...
void ZJpsi_onlyMC_ee_rec::analyzeDecay(const reco::Candidate* mother, TLorentzVector& Z, TLorentzVector& l1, TLorentzVector& l2, TLorentzVector& m1, TLorentzVector& m2, TLorentzVector& dim, int indent = 0){
...
bool ZJpsi_onlyMC_ee_rec::isAncestor(const reco::Candidate* ancestor, const reco::Candidate * particle) {
...
bool ZJpsi_onlyMC_ee_rec::isAncestor(int a_pdgId, const reco::Candidate * particle) {
...
void 
ZJpsi_onlyMC_ee_rec::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
ZJpsi_onlyMC_ee_rec::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
ZJpsi_onlyMC_ee_rec::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(ZJpsi_onlyMC_ee_rec);
